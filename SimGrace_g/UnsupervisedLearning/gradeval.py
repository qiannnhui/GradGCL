...
def get_grad(embed, loss):
    grads = torch.autograd.grad(loss,
                                ...
                                only_inputs=True)
    grad = grads[0]
    # print('before',grad.size())
    # torch.nn.functional.normalize(grad, dim=0)
        ...
        for epoch in range(1, epochs + 1):
            ...
            for data in dataloader:
                ...
                if a == 0.0:
                    ...
                    lossg = model.loss_cal(grads2, grads1, args)                        # a = 0.4
            ...
            if epoch % args.log_interval == 0:
                model.eval()
                #emb, y = model.encoder.get_embeddings(dataloader_eval)
                X, y = model.encoder.get_embeddings_grad(dataloader_eval)
                X2 = gen_ran_output_eval(dataloader_eval, model, vice_model, args)
                lossf = model.loss_cal(X2, X, args)
                # if a != 0.0:
                grads_eval = get_grad(X, lossf)
                #y = y.detach()
                X = X.detach()
                grads_eval = grads_eval.detach()
                #emb = X + a*grads_eval
                emb = torch.cat((X,grads_eval),0)
                acc_val, acc = evaluate_embedding(emb.cpu(), y)
                # accuracies['val'].append(acc_val)
                # accuracies['test'].append(acc)
                if acc > best_acc:
                    best_acc = acc
...
if __name__ == '__main__':
    ...
    for a in ass:
        ...
        mean.append(p.mean())
        std.append(p.std())
    with open('logs/log_gradeval' + args.DS + '_' + str(args.eta), 'a+') as f:
    ...
    #plt.savefig('/export/data/rli/Project/new/Simg/unsupervised_TU/result/' + 'result_two' + '_' + args.DS +'_'+str(args.eta)+'.png')
    plt.show()

...
def main(argv):
    ...
    def train(step):
        ...
        lossf = 2 - cosine_similarity(q1, y2.detach(), dim=-1).mean() - cosine_similarity(q2, y1.detach(), dim=-1).mean()
        a = 0.15
        if a == 0.0:
            loss = lossf
        else:
            grads_q1 = get_grad(q1, lossf)
            grads_q2 = get_grad(q2, lossf)
            lossg = 2 - cosine_similarity(grads_q1, y2.detach(), dim=-1).mean() - cosine_similarity(grads_q2, y1.detach(), dim=-1).mean()

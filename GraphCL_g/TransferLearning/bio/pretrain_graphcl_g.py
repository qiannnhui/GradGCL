...
def get_grad(embed, loss, args):
    grads = torch.autograd.grad(loss,
                                        embed,
                                        create_graph=True,
                                        only_inputs=True)
    grad = grads[0]
    embedding_dim = args.emb_dim * args.num_layer
    proj_head = nn.Sequential(nn.Linear(embedding_dim, embedding_dim), nn.ReLU(inplace=True),
                                   nn.Linear(embedding_dim, embedding_dim)).to(args.device)
    #grad = proj_head(grad)
...
def train(args, model, device, dataset, optimizer):
    ...
    for step, batch in enumerate(tqdm(zip(loader1, loader2), desc="Iteration")):
        ...
        if a == 0.0:
            ...
            grads1, grads2 = get_grad(x1, lossf, args), get_grad(x2, lossf, args)
...
def main():
    ...
    for epoch in range(1, args.epochs):
        ...
        if epoch % 20 == 0:
            torch.save(model.gnn.state_dict(), "./models_graphcl/graphcl_" + str(args.a) +'_' + str(epoch) + ".pth")

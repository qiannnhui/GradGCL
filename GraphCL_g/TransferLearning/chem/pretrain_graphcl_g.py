...
def get_grad(embed, loss, device):
    ...
    proj_head = nn.Sequential(nn.Linear(300, 300), nn.ReLU(inplace=True), nn.Linear(300, 300)).to(device)
    grad = proj_head(grad)
    return grad
'''
...
def train(args, model, device, dataset, optimizer):
    ...
    for step, batch in enumerate(tqdm(zip(loader1, loader2), desc="Iteration")):
        ...
        if a == 0.0:
            ...
            grads1, grads2 = get_grad(x1, lossf,device), get_grad(x2, lossf,device)
...
def main():
    ...
    for epoch in range(1, args.epochs):
        ...
        if epoch % 20 == 0:
            torch.save(gnn.state_dict(), "./models_graphcl/graphcl_" + str(args.a) +'_'+ str(epoch) + ".pth")

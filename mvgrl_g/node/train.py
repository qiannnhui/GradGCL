...
class Model(nn.Module):
    ...
    def forward(self, seq1, seq2, adj, diff, sparse, msk, samp_bias1, samp_bias2, lbl):
...
def train(dataset, a, verbose=False):
    ...
    with tqdm(total=nb_epochs, desc='(T)') as pbar:
        for epoch in range(nb_epochs):
            ...
            lossf = model(bf, shuf_fts, ba, bd, sparse, None, None, None , lbl)

            # lossf = model.loss_cl(features, shuf_fts, sp_adj if sparse else adj, sparse, None, None, None)

            if a == 0.0:
                loss = lossf
            else:
                grads1, grads2 = get_grad(bf, lossf), get_grad(shuf_fts, lossf)
                lossg = model(grads1, grads2, ba, bd, sparse, None, None, None , lbl)
                # lossg = b_xent(logits_g, lbl)
                # a = 0.4
    ...
    accs = torch.stack(accs)
    mean = accs.mean().cpu().numpy()
    std = accs.std().cpu().numpy()
    print(mean)
    print(std)
    return (mean)
...
if __name__ == '__main__':
    ...
    #input the gradient weight
    a = 0.2
    p = []
    for i in range(1, 5):
        p.append(train(dataset, a))
        print('Now is in', i, ',a is', a)
    p = np.array(p)
    mean.append(p.mean())
    std.append(p.std())

    with open('logs/log_' + 'pubmed', 'a+') as f:
        s = json.dumps([str(mean),str(std)])
        f.write('{},{}\n'.format('pubmed', s))
        f.write('\n')
